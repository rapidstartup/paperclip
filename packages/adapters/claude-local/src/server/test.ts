...
function resolveClaudeRunAsUser(config: Record<string, unknown>): { uid: number; gid: number } | null {
    ...
    if (!commandLooksLike(command, "claude")) {
      ...
      if ((dangerouslySkipPermissions || hasDangerousSkipInExtraArgs) && effectiveRunAsRoot) {
        checks.push({
          ...
          level: "warn",
          message:
            "Ignoring dangerouslySkipPermissions because Claude CLI forbids --dangerously-skip-permissions under root/sudo.",
          hint:
            "Run the adapter process as a non-root user to use unattended permission bypass with Claude.",
        });
      }
