...
import { asBoolean, asNumber, asString, asStringArray, buildPaperclipEnv, ensureAbsoluteDirectory, ensureCommandResolvable, ensurePathInEnv, parseObject, redactEnvForLogs, renderTemplate, runChildProcess, } from "@paperclipai/adapter-utils/server-utils";
import { DEFAULT_AGENT_BROWSER_COMMAND, DEFAULT_AGENT_BROWSER_SESSION_TEMPLATE, DEFAULT_AGENT_BROWSER_SUBCOMMAND, DEFAULT_AGENT_BROWSER_SUBCOMMAND_ARGS, } from "../index.js";
import { firstNonEmptyLine, isAgentBrowserUnknownSessionError, parseAgentBrowserOutput, } from "./parse.js";
const DEFAULT_PROMPT = "You are agent {{agent.id}} ({{agent.name}}). Continue your Paperclip browser automation work.";
...
function parseSubcommandArgs(config) {
...
function resolveSessionName(config, runtimeSessionParams, runtime, cwd, templateData) {
    const configuredTemplate = asString(config.sessionNameTemplate, "").trim();
    const runtimeSessionName = asString(runtimeSessionParams.sessionName, "") ||
    ...
    const canReuseRuntimeSession = runtimeSessionName.length > 0 &&
...
function resolveTemplatePath(candidateTemplate, cwd, templateData) {
    if (!candidateTemplate.trim())
        return null;
...
async function loadInstructionsPrefix(instructionsFilePath, onLog) {
    if (!instructionsFilePath)
        return "";
    const instructionsDir = `${path.dirname(instructionsFilePath)}${path.sep}`;
    try {
        const instructionsContents = await fs.readFile(instructionsFilePath, "utf8");
        await onLog("stderr", `[paperclip] Loaded agent-browser instructions file: ${instructionsFilePath}\n`);
...
export async function execute(ctx) {
    ...
    const subcommandArgs = (() => {
        ...
        if (subcommand === "snapshot")
            return [...DEFAULT_AGENT_BROWSER_SUBCOMMAND_ARGS];
        return [];
    })();
    const extraArgs = (() => {
    ...
    if (provider)
        env.PAPERCLIP_AGENT_BROWSER_PROVIDER = provider;
    if (sessionName)
        env.PAPERCLIP_AGENT_BROWSER_SESSION = sessionName;
    if (profileDir)
        env.PAPERCLIP_AGENT_BROWSER_PROFILE = profileDir;
    if (workspaceCwd)
    ...
    const wakeReason = typeof context.wakeReason === "string" && context.wakeReason.trim().length > 0
        ...
        : null;
    const wakeCommentId = (typeof context.wakeCommentId === "string" &&
    ...
    const buildArgs = (withSession) => {
        const args = [];
        if (provider)
            args.push("-p", provider);
        if (withSession && sessionName)
            args.push("--session-name", sessionName);
        if (profileDir)
            args.push("--profile", profileDir);
        if (actionPolicyPath)
            args.push("--action-policy", actionPolicyPath);
        if (allowedDomains.length > 0)
            args.push("--allowed-domains", allowedDomains.join(","));
        if (headed)
            args.push("--headed");
        if (contentBoundaries)
            args.push("--content-boundaries");
        if (maxOutput > 0)
            args.push("--max-output", String(maxOutput));
        ...
        if (targetUrl)
            args.push(targetUrl);
        if (appendPromptAsArg && prompt.trim().length > 0)
            args.push(prompt);
        return args;
    };
    const commandNotes = [
    ...
    const runAttempt = async (withSession) => {
    ...
    const toResult = (attempt, clearSessionOnRetry) => {
        ...
        const fallbackError = attempt.parsed.errorMessage ||
            firstNonEmptyLine(attempt.proc.stderr) ||
            `agent-browser exited with code ${attempt.proc.exitCode ?? -1}`;
        const resolvedSessionParams = sessionName && attempt.args.includes("--session-name")
            ? {
                sessionName,
                cwd,
                ...(provider ? { provider } : {}),
    ...
    if (sessionName &&
        ...
        await onLog("stderr", `[paperclip] agent-browser session "${sessionName}" is unavailable; retrying without session state.\n`);
