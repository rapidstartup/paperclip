...
export const sessionCodec: AdapterSessionCodec = {
  deserialize(raw: unknown) {
    ...
    const sessionName =
      readNonEmptyString(record.sessionName) ??
    ...
    const profileDir =
      readNonEmptyString(record.profileDir) ?? readNonEmptyString(record.profile_dir);
    return {
      sessionName,
      ...(cwd ? { cwd } : {}),
      ...(provider ? { provider } : {}),
  ...
  serialize(params: Record<string, unknown> | null) {
    ...
    const sessionName =
      readNonEmptyString(params.sessionName) ??
    ...
    const profileDir =
      readNonEmptyString(params.profileDir) ?? readNonEmptyString(params.profile_dir);
    return {
      sessionName,
      ...(cwd ? { cwd } : {}),
      ...(provider ? { provider } : {}),
