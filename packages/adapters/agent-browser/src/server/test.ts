...
function providerKeyFor(provider: string): string | null {
  const normalized = provider.trim().toLowerCase();
  if (!normalized) return null;
  return PROVIDER_ENV_KEYS[normalized] ?? null;
}

function summarizeProbeError(stdout: string, stderr: string): string | null {
...
export async function testEnvironment(
  ...
  if (!provider) {
    ...
    if (!requiredEnvKey) {
      ...
      typeof runtimeEnv[requiredEnvKey] === "string" &&
  ...
  const canRunProbe = checks.every(
    (check) =>
      check.code !== "agent_browser_cwd_invalid" &&
      check.code !== "agent_browser_command_unresolvable",
  );

  if (canRunProbe) {
    const probe = await runChildProcess(
      `agent-browser-envtest-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      command,
      ["--help"],
      {
        cwd,
