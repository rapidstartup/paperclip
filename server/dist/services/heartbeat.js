...
import { AGENT_DEFAULT_MAX_CONCURRENT_RUNS, ISSUE_CONTINUATION_SUMMARY_DOCUMENT_KEY, } from "@paperclipai/shared";
import { agents, agentRuntimeState, agentTaskSessions, agentWakeupRequests, activityLog, companies, companySkills as companySkillsTable, documentRevisions, issueDocuments, heartbeatRunEvents, heartbeatRuns, issueComments, issueRelations, issues, issueWorkProducts, projects, projectWorkspaces, workspaceOperations, } from "@paperclipai/db";
...
import { classifyIssueGraphLiveness, } from "./issue-liveness.js";
...
import { RUN_LIVENESS_CONTINUATION_REASON, buildRunLivenessContinuationIdempotencyKey, decideRunLivenessContinuation, findExistingRunLivenessContinuationWake, readContinuationAttempt, } from "./run-continuations.js";
...
export async function resolveExecutionRunAdapterConfig(input) {
    const { config: resolvedConfig, secretKeys } = await input.secretsSvc.resolveAdapterConfigForRuntime(input.companyId, input.executionRunConfig);
 ...
 */
function injectGitCloneToken(repoUrl, companyToken = null) {
...
function allowsBlockedIssueInteractionWake(contextSnapshot) {
    const wakeReason = readNonEmptyString(contextSnapshot?.wakeReason);
    if (!wakeReason || !BLOCKED_INTERACTION_WAKE_REASONS.has(wakeReason))
        return false;
    ...
    return {
        ...
        continuationSummary: continuationSummary
            ? {
                ...
                body: continuationSummary.body.length > 4_000
...
export function heartbeatService(db) {
    ...
    async function scheduleBoundedRetryForRun(run, agent, opts) {
        ...
        const codexTransientFallbackMode = agent.adapterType === "codex_local" && retryReason === BOUNDED_TRANSIENT_HEARTBEAT_RETRY_REASON && run.errorCode === "codex_transient_upstream"
    ...
    async function claimQueuedRun(run) {
        ...
        if (claimedIssueId) {
            ...
            await db
                ...
                .where(and(eq(issues.id, claimedIssueId), eq(issues.companyId, claimed.companyId), or(isNull(issues.executionRunId), eq(issues.executionRunId, claimed.id))));
        }
        return claimed;
    }
    ...
    async function findOpenLivenessEscalation(companyId, incidentKey) {
        return db
            .select()
            .from(issues)
            .where(and(eq(issues.companyId, companyId), eq(issues.originKind, "harness_liveness_escalation"), eq(issues.originId, incidentKey), isNull(issues.hiddenAt), notInArray(issues.status, ["done", "cancelled"])))
            .limit(1)
            .then((rows) => rows[0] ?? null);
    }
    ...
    async function releaseIssueExecutionAndPromote(run) {
        ...
        const promotionResult = await db.transaction(async (tx) => {
            if (contextIssueId) {
                await tx.execute(sql `select id from issues where company_id = ${run.companyId} and id = ${contextIssueId} for update`);
            }
            else {
                await tx.execute(sql `select id from issues where company_id = ${run.companyId} and execution_run_id = ${run.id} for update`);
            }
            let issue = await tx
                ...
                .from(issues)
                .where(and(eq(issues.companyId, run.companyId), contextIssueId ? eq(issues.id, contextIssueId) : eq(issues.executionRunId, run.id)))
                .then((rows) => rows[0] ?? null);
            if (!issue)
                return null;
            ...
            while (true) {
                ...
                const shouldReopenDeferredCommentWake = deferredCommentIds.length > 0 && (issue.status === "done" || issue.status === "cancelled");
