...
function resolveWorktreeRuntimeContext(env, overrideConfigPath) {
    ...
    const worktreeName = nonEmpty(persistedEnv.PAPERCLIP_WORKTREE_NAME) ??
        nonEmpty(env.PAPERCLIP_WORKTREE_NAME) ??
        path.basename(worktreeRoot);
    const instanceId = nonEmpty(persistedEnv.PAPERCLIP_INSTANCE_ID) ??
        nonEmpty(env.PAPERCLIP_INSTANCE_ID) ??
        sanitizeWorktreeInstanceId(worktreeName);
    const homeDir = resolveHomeAwarePath(nonEmpty(persistedEnv.PAPERCLIP_HOME) ??
