...
import { PERMISSION_KEYS, createVibedashProjectBootstrapSchema, } from "@paperclipai/shared";
import { and, eq } from "drizzle-orm";
import { conflict, forbidden, unauthorized } from "../errors.js";
import { validate } from "../middleware/validate.js";
import { accessService, agentService, approvalService, companyService, heartbeatService, issueService, logActivity, projectService, secretService, } from "../services/index.js";
...
const DEFAULT_CUSTOMER_BOARD_GRANTS = [...PERMISSION_KEYS];
...
function slugifyWorkspaceSegment(value) {
...
async function reserveWorkspacePath(rootPath, projectName) {
...
function withGitHubAccessToken(repoUrl, token) {
    if (!token)
        return repoUrl;
    try {
...
function sanitizeGitErrorText(text, token) {
...
async function runGit(input) {
    try {
        ...
        throw conflict(`${input.operation} failed for ${input.safeRepoUrl}${detail ? `: ${detail}` : ""}`);
    }
}
async function prepareWorkspaceRepository(input) {
    ...
    if (!hasGitDir) {
        ...
        if (targetExists) {
            ...
            if (entries.length > 0) {
                throw conflict(`Workspace path ${targetDir} already exists and is not an empty git checkout.`);
            }
...
function normalizeBoardGrants(input) {
...
function resolveBootstrapActor(req) {
...
export function vibedashRoutes(db) {
    const router = Router();
    ...
    router.post("/integrations/vibedash/bootstrap-project", validate(createVibedashProjectBootstrapSchema), async (req, res) => {
        ...
        if (existingLink && existingLink.bootstrapStatus !== "ready") {
            throw conflict(`Vibedash project ${input.vibedashProjectId} already has a bootstrap record in status '${existingLink.bootstrapStatus}'.`);
        }
        ...
        const ensureOwnerWithDefaultGrants = async (userId) => {
            await accessSvc.ensureMembership(company.id, "user", userId, "owner", "active");
            await accessSvc.setPrincipalGrants(company.id, "user", userId, normalizeBoardGrants(input.customerBoardGrants), actor.userId);
        };
        if (actor.userId) {
        ...
        const [link] = await db
            ...
            .returning();
        try {
            const workspaceRoot = readNonEmptyString(input.workspaceRootHint) ??
                path.resolve(resolvePaperclipInstanceRoot(), "workspaces");
            const workspaceCwd = await reserveWorkspacePath(workspaceRoot, input.vibedashProjectName);
            ...
            if (!workspace)
                throw conflict("Failed to create project workspace");
            const vibedashSecret = await secretsSvc.create(company.id, {
            ...
            const githubSecret = githubToken
                ? await secretsSvc.create(company.id, {
                    name: "github_token",
                    provider: "local_encrypted",
                    value: githubToken,
            ...
            const requestedAdapterConfig = pmAgentInput.adapterConfig ?? {};
            const requestedRuntimeConfig = pmAgentInput.runtimeConfig ?? {};
            const requestedEnv = (() => {
            ...
            const normalizedAdapterConfig = await secretsSvc.normalizeAdapterConfigForPersistence(company.id, pmAdapterConfig, { strictMode: strictSecretsMode });
            const requiresApproval = input.requireBoardApprovalForNewAgents;
            const createdAgent = await agentsSvc.create(company.id, {
                name: readNonEmptyString(pmAgentInput.name) ??
                    `Vibedash PM - ${input.vibedashProjectName}`,
                role: "pm",
                title: "Product Manager",
                status: requiresApproval ? "pending_approval" : "idle",
                reportsTo: null,
                capabilities: readNonEmptyString(pmAgentInput.capabilities) ??
            ...
            if (requiresApproval) {
                ...
                const approverUserId = input.customerBoardUserId ?? actor.userId ?? "vibedash-bootstrap";
                await approvalsSvc.approve(approval.id, approverUserId, "Auto-approved as part of Vibedash project bootstrap");
            ...
            await accessSvc.setPrincipalGrants(company.id, "agent", pmAgent.id, [
            ...
            const [readyLink] = await db
                ...
                .where(and(eq(vibedashProjectLinks.id, link.id), eq(vibedashProjectLinks.companyId, company.id)))
                .returning();
