...
function buildGitAuthArgs(repoUrl: string) {
  if (!isGitHubUrl(repoUrl)) return [] as string[];
...
function parseGitError(op: string, err: unknown): Error {
  ...
  if (!detail) return new Error(`${op} failed.`);
  return new Error(`${op} failed: ${detail}`);
}

async function runGit(repoDir: string, repoUrl: string, op: string, args: string[]) {
...
async function resolveDefaultOriginBranch(repoDir: string, repoUrl: string): Promise<string | null> {
  try {
    const output = await runGit(
      repoDir,
      repoUrl,
      "Resolve default origin branch",
...
function resolveManagedRepoDir(companyId: string, projectId: string, workspaceId: string) {
...
async function withWorkspaceLock(
  lockKey: string,
  fn: () => Promise<EnsureWorkspaceRepoResult>,
...
export async function ensureWorkspaceRepo(input: EnsureWorkspaceRepoInput): Promise<EnsureWorkspaceRepoResult> {
  ...
  return withWorkspaceLock(lockKey, async () => {
    ...
    if (!hasGitDir) {
      ...
      await runGit(
        parentDir,
        repoUrl,
        "Clone project workspace repository",
      ...
      await runGit(
        repoDir,
        repoUrl,
        "Update project workspace repository remote",
        ["-C", repoDir, "remote", "set-url", DEFAULT_REMOTE, repoUrl],
      );
    }
    ...
    await runGit(
      repoDir,
      repoUrl,
      "Fetch project workspace repository",
      ["-C", repoDir, "fetch", "--prune", DEFAULT_REMOTE],
    );
    ...
    if (repoRef) {
      await runGit(
        repoDir,
        repoUrl,
        "Checkout configured project workspace ref",
        ["-C", repoDir, "checkout", "--force", repoRef],
      );
      try {
        await runGit(
          repoDir,
          repoUrl,
          "Hard reset project workspace branch",
          ["-C", repoDir, "reset", "--hard", `${DEFAULT_REMOTE}/${repoRef}`],
        );
      } catch {
    ...
    await runGit(
      repoDir,
      repoUrl,
      "Checkout project workspace default branch",
      ["-C", repoDir, "checkout", "--force", defaultBranch],
    );
    await runGit(
      repoDir,
      repoUrl,
      "Reset project workspace to origin/default branch",
      ["-C", repoDir, "reset", "--hard", `${DEFAULT_REMOTE}/${defaultBranch}`],
    );
    ...
    const githubHint = isGitHubUrl(input.repoUrl)
      ...
      : "";
    throw new Error(
      `Failed to materialize repo workspace from "${redactRepoUrl(input.repoUrl)}": ${reason}.${githubHint}`,
    );
  });
}
